...
public class RegisterController { 
   ...
   @PostMapping("/add")
   public String save(@Valid User user, BindingResult result, Model m) throws UnsupportedEncodingException, SQLException {
      //1.유효성 검사 -> 관심사로 분리
	   System.out.println("result = " + result);
	   System.out.println("user = " + user);
	   
	  //2.DB에 새회원 정보를 저장
	  //User객체 검증한 결과 에러가 있으면, registerForm.jsp를 이용해서 에러를 보여줘야 함
	  if(!result.hasErrors()) {
